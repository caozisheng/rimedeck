...
const defaultCLIConfigPath = ".rimedeck/config.json"
...
// An empty profile returns the default path (~/.rimedeck/config.json).
// A named profile returns ~/.rimedeck/profiles/<name>/config.json.
func CLIConfigPathForProfile(profile string) (string, error) {
	...
	return filepath.Join(home, ".rimedeck", "profiles", profile, "config.json"), nil
}

// ProfileDir returns the base directory for a profile's state files (pid, log).
// An empty profile returns ~/.rimedeck/. A named profile returns ~/.rimedeck/profiles/<name>/.
func ProfileDir(profile string) (string, error) {
	...
	if profile == "" {
		return filepath.Join(home, ".rimedeck"), nil
	}
	return filepath.Join(home, ".rimedeck", "profiles", profile), nil
}
