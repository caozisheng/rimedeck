...
func formatProjectResource(r ProjectResourceForEnv) string {
	...
	case "gitlab_repo":
		...
		_ = json.Unmarshal(r.ResourceRef, &payload)
		out := fmt.Sprintf("**GitLab repo**: %s", payload.URL)
		if payload.DefaultBranchHint != "" {
			out += fmt.Sprintf(" (default branch: `%s`)", payload.DefaultBranchHint)
		}
		if label != "" {
...
// For OMP:      writes {workDir}/AGENTS.md  (skills discovered natively from .omp/skills/)
...
// For Qoder:       writes {workDir}/AGENTS.md  (Qoder CLI reads AGENTS.md natively)
// For Qwen Code:   writes {workDir}/AGENTS.md  (qwen-code reads AGENTS.md natively)
...
func runtimeConfigPath(workDir, provider string) string {
	...
	case "codex", "copilot", "opencode", "openclaw", "hermes", "pi", "omp", "cursor", "kimi", "kiro", "antigravity", "qoder", "qwencode":
...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	if len(ctx.AgentSkills) > 0 {
		...
		case "codex", "copilot", "opencode", "openclaw", "pi", "omp", "cursor", "kimi", "kiro", "antigravity":
