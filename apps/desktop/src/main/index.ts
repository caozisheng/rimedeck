...
if (process.platform === "win32") {
  // Windows GUI launches (Explorer, Start Menu) inherit the full user PATH,
  // but Electron packaged apps sometimes lose user-level entries. Prepend
  // common install locations for agent CLIs (claude, codex, opencode).
  const home = homedir();
  ...
  process.env.PATH = `${fallbackPaths.join(";")}${process.env.PATH ? ";" + process.env.PATH : ""}`;
} else {
  fixPath();
...
function handleDeepLink(url: string): void {
  try {
    ...
    // rimedeck://auth/callback?token=<jwt>
...
// single-worktree case still lands at "RimeDeck Canary".
const DEV_APP_NAME = process.env.DESKTOP_APP_SUFFIX
  ? `RimeDeck Dev ${process.env.DESKTOP_APP_SUFFIX}`
  : "RimeDeck Dev";
...
if (is.dev) {
  ...
  // to "RimeDeck", but anchoring it here makes WM_CLASS ↔ StartupWMClass
