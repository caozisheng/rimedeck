...
Local development uses one shared PostgreSQL instance and one database per checkout.
...
- Isolation happens at the database level; each checkout gets its own database
...
- Ensures the shared PostgreSQL instance is running
...
This is a first-class workflow. Both checkouts use the same PostgreSQL instance but different databases and ports:
...
# (method depends on how you installed PostgreSQL)
```

> **Warning:** This deletes all databases. After that you must run `make setup-main` or `make setup-worktree` again.
