...
```bash
grep "EmailService:" <path-to-server-log>
# or check the server's stdout directly if running in the foreground
```

If the line you expected isn't there, the environment didn't reach the process — check `.env` and verify the environment variables are set in the server process (check server logs at startup for the `EmailService:` line). Credentials are never logged on this startup line.
...
|---|---|---|
| `smtp dial <host>:<port>: dial tcp …: connect: connection refused` / `i/o timeout` | The backend can't reach the relay — wrong host, wrong port, firewall, or the relay isn't listening | Verify `SMTP_HOST` / `SMTP_PORT` resolve from the server host (`nslookup <host>` and `nc -vz <host> <port>`); open the firewall from the host running Multica to the relay |
...
- From the server host, sanity-check connectivity: `nc -vz $SMTP_HOST $SMTP_PORT`
- Confirm the env reached the process by checking server logs at startup for the `EmailService:` line
...
# or check server logs to see which values the process received
```
...
- If you have legacy `pg_cron` history that pre-dates the in-process scheduler, the SQL function still holds advisory lock 4246 internally and the two paths cannot double-write — the optional `cron.unschedule` cleanup is described in the server's environment variables documentation.
...
2. Re-run the upgrade — restart the backend so migrations run. The guard now sees a current watermark and lets `103` apply.
3. The in-process scheduler then keeps the watermark advancing.
...
| **Server** | stdout / journal depending on how you run it | `journalctl -u multica-server -f` or check stdout directly |
