...
```bash
grep "EmailService:" <服务器日志路径>
# 或者在前台运行时直接查看 stdout
```

如果应该出现的那行没出现，说明环境变量没进到进程 —— 检查 `.env` 并查看服务器启动时的日志是否出现 `EmailService:` 这行。这行启动日志里**不会**打印任何密码。
...
| 错误日志 | 含义 | 怎么修 |
|---|---|---|
| `smtp dial <host>:<port>: dial tcp …: connect: connection refused` / `i/o timeout` | backend 连不上 relay —— host / port 错、防火墙挡了、或者 relay 没开 | 从服务器主机上确认能解析和连通：`nslookup <host>` 以及 `nc -vz <host> <port>`；放行从 Multica 主机到 relay 的网络 |
...
- 从服务器主机上测连通：`nc -vz $SMTP_HOST $SMTP_PORT`
- 通过查看服务器启动日志中的 `EmailService:` 行确认环境变量真进到了进程
...
- 历史里有遗留的 `pg_cron` 入口也没事 —— SQL 函数里还持有 advisory lock 4246，应用调度器和 `pg_cron` 不会双写；要清掉冗余项的方法见服务器环境变量文档。
...
2. 重新跑升级 —— 重启 backend 即可，启动时会自动跑 migration。Guard 看到新的 watermark，`103` 就会通过。
3. 之后由进程内调度器持续推 watermark。
...
| **服务器** | 根据运行方式，stdout 或 journal | `journalctl -u multica-server -f` 或直接查看 stdout |
