...
```bash
grep "EmailService:" <서버-로그-경로>
# 또는 서버가 포그라운드에서 실행 중이라면 stdout을 직접 확인하세요
```

기대했던 줄이 보이지 않는다면 환경 변수가 프로세스에 도달하지 않은 것입니다 — `.env`를 확인하고 서버 시작 시 `EmailService:` 줄이 로그에 표시되는지 확인하세요. 이 시작 로그 줄에는 자격 증명이 절대 기록되지 않습니다.
...
| 기록된 오류 | 의미 | 해결 방법 |
|---|---|---|
| `smtp dial <host>:<port>: dial tcp …: connect: connection refused` / `i/o timeout` | 백엔드가 relay에 도달할 수 없음 — 잘못된 host, 잘못된 port, 방화벽, 또는 relay가 수신 대기 중이 아님 | 서버 호스트에서 `SMTP_HOST` / `SMTP_PORT`가 해석되는지 확인하세요(`nslookup <host>` 및 `nc -vz <host> <port>`). Multica를 실행하는 호스트에서 relay로 향하는 방화벽을 여세요 |
...
- 서버 호스트에서 연결성을 점검하세요: `nc -vz $SMTP_HOST $SMTP_PORT`
- 서버 시작 시 `EmailService:` 로그 줄을 확인하여 환경 변수가 프로세스에 도달했는지 확인하세요
...
- 인프로세스 스케줄러 이전의 레거시 `pg_cron` 이력이 있어도 SQL 함수가 내부적으로 advisory lock 4246을 잡고 있어 두 경로가 이중 쓰기할 수 없습니다 — 선택적 `cron.unschedule` 정리 방법은 서버 환경 변수 문서를 참고하세요.
...
2. 업그레이드를 다시 실행하세요 — 백엔드를 재시작하면 마이그레이션이 실행됩니다. 이제 가드가 최신 워터마크를 확인하고 `103`을 적용하도록 허용합니다.
3. 인프로세스 스케줄러가 이후 워터마크를 계속 진행시킵니다.
...
| **서버** | 실행 방식에 따라 stdout 또는 journal | `journalctl -u multica-server -f` 또는 stdout 직접 확인 |
