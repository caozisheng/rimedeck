...
```bash
grep "EmailService:" <サーバーログのパス>
# またはフォアグラウンドで実行中の場合は stdout を直接確認してください
```

期待していた行が見当たらない場合は、環境変数がプロセスに届いていません。`.env` を確認し、サーバー起動時に `EmailService:` 行がログに表示されるか確認してください。この起動ログの行には認証情報は一切記録されません。
...
| 記録されたエラー | 意味 | 解決方法 |
|---|---|---|
| `smtp dial <host>:<port>: dial tcp …: connect: connection refused` / `i/o timeout` | バックエンドが relay に到達できない — host が誤っている、port が誤っている、ファイアウォール、または relay が待ち受けていない | サーバーホストから `SMTP_HOST` / `SMTP_PORT` が解決されるか確認してください（`nslookup <host>` および `nc -vz <host> <port>`）。Multica を実行するホストから relay へのファイアウォールを開放してください |
...
- サーバーホストから接続性を点検してください: `nc -vz $SMTP_HOST $SMTP_PORT`
- サーバー起動時の `EmailService:` ログ行を確認して、環境変数がプロセスに届いたか確認してください
...
# またはサーバーログでプロセスが受け取った値を確認してください
```
...
- インプロセススケジューラー以前のレガシー `pg_cron` 履歴がある場合でも、SQL 関数は内部で advisory lock 4246 を保持するため二重書き込みは発生しません — オプションの `cron.unschedule` クリーンアップについてはサーバーの環境変数ドキュメントを参照してください。
...
2. アップグレードを再実行してください — バックエンドを再起動するだけで十分で、マイグレーションは起動時に実行されます。これでガードが最新のウォーターマークを確認し、`103` の適用を許可します。
3. インプロセススケジューラーがウォーターマークを進め続けます。
...
| **サーバー** | 実行方法によって stdout または journal | `journalctl -u multica-server -f` または stdout を直接確認 |
