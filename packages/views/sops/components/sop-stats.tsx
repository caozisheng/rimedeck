...
import { sopStatsOptions } from "@rimedeck/core/workspace/queries";
...
export function SOPStats({ sopId }: SOPStatsProps) {
  const wsId = useWorkspaceId();
  const [expanded, setExpanded] = useState(false);
  const { t } = useT("sops");

  const { data: stats } = useQuery(sopStatsOptions(wsId, sopId));
