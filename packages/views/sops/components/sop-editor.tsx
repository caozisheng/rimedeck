...
import './sop-canvas.css';
import { useQuery, useQueryClient } from '@tanstack/react-query';
import { useWorkspaceId } from '@rimedeck/core/hooks';
import { agentListOptions, sopRunKeys } from '@rimedeck/core/workspace/queries';
...
export function SOPEditor({
  ...
  const handleSave = useCallback(() => {
    const dsl = toRuleGoDSL(nodes as Node<SOPNodeData>[], edges, chainInfo);
    onChange?.(dsl);
  ...
  const selectedNode = selectedNodeId
    ? (nodes.find((n) => n.id === selectedNodeId) as Node<SOPNodeData> | undefined) ?? null
    : null;

  const updateSelectedNode = useCallback(
    (updater: (data: SOPNodeData) => SOPNodeData) => {
      if (!selectedNodeId) return;
      setNodes((nds) =>
        nds.map((n) =>
          n.id === selectedNodeId ? { ...n, data: updater(n.data as SOPNodeData) } : n,
        ),
      );
    },
  ...
  const handleTestRun = useCallback(async () => {
    ...
    try {
      ...
      const run = await api.triggerSOPRun(sopId, testAgentId || undefined);
  ...
  }, [sopId, triggering, testAgentId, nodes, edges, chainInfo, onChange, wsId, qc, t]);
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div className="flex-1 relative">
          <ReactFlow
            ...
            className="bg-background sop-canvas"
          >
            ...
            <div className="max-h-72 overflow-y-auto border-t">
              ...
              {activeRunId && (
                <div className="border-b bg-accent/5">
                  ...
                  <RunMonitor runId={activeRunId} sopId={sopId} />
                </div>
              )}
              <RunHistory sopId={sopId} />
            </div>
          </CollapsibleContent>
        </div>
      </Collapsible>
