...
import type { Agent, SOPSummary } from "@rimedeck/core/types";
...
// ---------------------------------------------------------------------------
// Constants moved from sops-page
// ---------------------------------------------------------------------------
...
export function useSOPColumns(
  onDelete: (wf: SOPSummary) => void,
): ColumnDef<SOPRow>[] {
  ...
  return [
    {
      ...
      meta: { grow: true },
      cell: ({ row }) => <SOPNameCell row={row.original} />,
    },
    {
      id: "usedBy",
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const cat = row.original.sop.category;
    ...
    },
    {
      ...
      cell: ({ row }) => <StatusBadge status={row.original.sop.status} />,
    },
    {
      id: "updated",
      ...
      cell: ({ row }) => (
        <span className="whitespace-nowrap text-xs text-muted-foreground">
          {timeAgo(row.original.sop.updated_at)}
        </span>
      ),
    },
    {
...
function SOPNameCell({ row }: { row: SOPRow }) {
  const { sop } = row;
  return (
