...
import { sopRunListOptions } from "@rimedeck/core/workspace/queries";
import type { SOPRun, SOPRunStatus } from "@rimedeck/core/types";
...
function StatusIcon({ status }: { status: SOPRunStatus }) {
  switch (status) {
    case "completed":
...
function formatDuration(run: SOPRun): string | null {
...
function RunRow({
  ...
  return (
    <div className="border-b last:border-b-0">
      ...
      {expanded && <RunMonitor runId={run.id} sopId={sopId} />}
    </div>
  );
}
...
export function RunHistory({ sopId }: RunHistoryProps) {
  const wsId = useWorkspaceId();
  const [expandedId, setExpandedId] = useState<string | null>(null);
  const { t } = useT("sops");

  const { data: runs, isLoading } = useQuery(
    sopRunListOptions(wsId, sopId),
  );
