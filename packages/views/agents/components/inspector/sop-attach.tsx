...
/**
 * Inline "+ Attach" trigger for the inspector's SOPs row. Mirrors the
 * SkillAttach component: a dashed-border chip that opens a dialog listing
 * unattached SOPs. Hidden when nothing is left to attach.
 */
export function SOPAttach({
  agent,
  canEdit = true,
}: {
  agent: Agent;
  ...
  const { data: workspaceSOPs = [] } = useQuery(sopListOptions(wsId));
  const [open, setOpen] = useState(false);

  const agentSOPIds = new Set((agent.sops ?? []).map((w) => w.id));
  const available = workspaceSOPs.filter(
    (w) => !agentSOPIds.has(w.id) && w.status !== "archived",
  );
  ...
  return (
    <>
      <button
        ...
        aria-label={t(($) => $.sop_attach.trigger_aria)}
        title={t(($) => $.sop_attach.trigger_aria)}
      ...
      >
        ...
        {t(($) => $.sop_attach.trigger_label)}
      </button>
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>
              {t(($) => $.sops.attach_dialog_title)}
            </DialogTitle>
          </DialogHeader>
          {available.length === 0 ? (
