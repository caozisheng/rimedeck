...
import { workspaceKeys, workflowTemplateListOptions } from "@rimedeck/core/workspace/queries";
import { api } from "@rimedeck/core/api";
import type { WorkflowTemplate } from "@rimedeck/core/types";
...
export function TemplateGallery({ onClose }: { onClose?: () => void }) {
  ...
  async function handleSelect(tmpl: WorkflowTemplate) {
    if (cloning) return;
    setCloning(tmpl.id);
    try {
      const workflow = await api.cloneWorkflowTemplate(tmpl.id);
      queryClient.invalidateQueries({
        queryKey: workspaceKeys.workflows(wsId),
      });
      toast.success(t(($) => $.template_gallery.created_from_template, { name: workflow.name }));
      onClose?.();
