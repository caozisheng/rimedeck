...
export function ImportDialog({
  ...
  async function handleImport() {
    ...
    setImporting(true);
    try {
      const result = await api.importWorkflow(raw);
      setWarnings(result.warnings);
      queryClient.invalidateQueries({
        queryKey: workspaceKeys.workflows(wsId),
      });
      toast.success(t(($) => $.import_dialog.imported, { name: result.workflow.name }));
      onOpenChange(false);
      reset();
      navigation.push(paths.workflowDetail(result.workflow.id));
    } catch {
