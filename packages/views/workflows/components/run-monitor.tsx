...
export function RunMonitor({ runId, workflowId }: RunMonitorProps) {
  const wsId = useWorkspaceId();
  const qc = useQueryClient();
  const { t } = useT("workflows");
  const runStatusLabel = (s: WorkflowRunStatus) =>
    (t(($) => $.run[s as keyof typeof $.run]) as string) ?? s;

  const isTerminal = (s: WorkflowRunStatus) =>
    s === "completed" || s === "failed" || s === "cancelled";

  const { data: run, isLoading } = useQuery({
    ...workflowRunDetailOptions(wsId, workflowId, runId, {
  ...
  const handleCancel = async () => {
    await api.cancelWorkflowRun(workflowId, runId);
