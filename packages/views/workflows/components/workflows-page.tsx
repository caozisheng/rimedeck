  ...
  return (
    <PageHeader className="justify-between px-5">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-sm font-medium">{tLayout(($) => $.nav.workflows)}</h1>
        {totalCount > 0 && (
        ...
        <DialogHeader>
          ...
          <DialogDescription
            dangerouslySetInnerHTML={{
              __html: t(($) => $.delete_dialog.description, { name: workflow.name }),
            }}
          />
        </DialogHeader>
        <DialogFooter>
...
export default function WorkflowsPage() {
  ...
  const assignments = useMemo(() => selectWorkflowAssignments(agents), [agents]);
  ...
  const columns = useWorkflowColumns((wf) => setDeleteTarget(wf));
  ...
  async function handleCreateBlank() {
    ...
    try {
      const wf = await api.createWorkflow({ name: t(($) => $.page.untitled_workflow) });
      queryClient.invalidateQueries({
        queryKey: workspaceKeys.workflows(wsId),
      });
      setCreateOpen(false);
      navigation.push(paths.workflowDetail(wf.id));
    } catch {
          ...
          <div className="flex flex-1 min-h-0 flex-col overflow-hidden rounded-lg border bg-background">
            ...
            {filtered.length === 0 ? (
              ...
              <DataTable
                table={table}
                onRowClick={(row) =>
                  navigation.push(paths.workflowDetail(row.original.workflow.id))
                }
              />
            )}
          </div>
