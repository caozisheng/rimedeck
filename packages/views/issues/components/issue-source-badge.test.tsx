...
describe("IssueSourceBadge", () => {
  it("renders a neutral Local badge without sync state", () => {
    renderWithI18n(<IssueSourceBadge issue={baseIssue} />);
  ...
  it("links a GitLab issue and renders pending sync state", () => {
    renderWithI18n(<IssueSourceBadge issue={{ ...baseIssue, source_type: "gitlab", sync_state: "pending", external: { provider: "gitlab", tracker_connection_id: "tracker-1", iid: 12, url: "https://gitlab.example/group/project/-/issues/12", author_name: null } }} />);
  ...
  it("shows failed and detached states", () => {
    const { rerender } = renderWithI18n(<IssueSourceBadge issue={{ ...baseIssue, source_type: "gitlab", sync_state: "failed" }} />);
