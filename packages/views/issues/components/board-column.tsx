...
import type { Issue, IssueAssigneeType, IssueStatus } from "@rimedeck/core/types";
...
import { STATUS_CONFIG } from "@rimedeck/core/issues/config";
import { useModalStore } from "@rimedeck/core/modals";
import { useViewStoreApi } from "@rimedeck/core/issues/stores/view-store-context";
...
export const BoardColumn = memo(function BoardColumn({
  ...
  return (
    <div style={{ minWidth: BOARD_COL_WIDTH, flex: `1 0 ${BOARD_COL_WIDTH}px` }} className={`flex flex-col rounded-xl ${cfg?.columnBg ?? "bg-muted/40"} p-2`}>
        ...
        >
          <SortableContext items={issueIds} strategy={verticalListSortingStrategy}>
            {resolvedIssues.map((issue) => (
              <DraggableBoardCard key={issue.id} issue={issue} childProgress={childProgressMap?.get(issue.id)} disableSorting={!!sortLabel} parentIssueMap={parentIssueMap} allIssues={allIssues} />
            ))}
          </SortableContext>
          {issueIds.length === 0 && (
