...
import { ApiError } from "@rimedeck/core/api";
...
vi.mock("@rimedeck/ui/components/ui/sidebar", () => ({
...
vi.mock("@rimedeck/ui/components/ui/collapsible", () => ({
...
vi.mock("@rimedeck/ui/components/common/actor-avatar", () => ({ ActorAvatar: () => <span /> }));
...
vi.mock("@rimedeck/core/api", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@rimedeck/core/api")>();
  return {
    ...actual,
    api: {
      ...actual.api,
...
vi.mock("@rimedeck/core/inbox/queries", () => ({ deduplicateInboxItems: (items: unknown[]) => items, inboxKeys: { list: () => ["inbox"] } }));
vi.mock("@rimedeck/core/issues/queries", () => ({ issueDetailOptions: () => ({ queryKey: ["issue"] }) }));
vi.mock("@rimedeck/core/issues/stores/create-mode-store", () => ({
  useCreateModeStore: { getState: () => ({ lastMode: "agent" }) },
  openCreateIssueWithPreference: vi.fn(),
}));
vi.mock("@rimedeck/core/issues/stores/draft-store", () => ({ useIssueDraftStore: () => false }));
vi.mock("@rimedeck/core/modals", () => ({ useModalStore: { getState: () => ({ modal: null, open: vi.fn() }) } }));
vi.mock("@rimedeck/core/pins/mutations", () => ({ useDeletePin: () => ({ mutate: deletePin }), useReorderPins: () => ({ mutate: vi.fn() }) }));
vi.mock("@rimedeck/core/pins/queries", () => ({ pinListOptions: () => ({ queryKey: ["pins"] }) }));
vi.mock("@rimedeck/core/projects/queries", () => ({ projectDetailOptions: () => ({ queryKey: ["project"] }) }));
vi.mock("@rimedeck/core/runtimes/hooks", () => ({ useMyRuntimesNeedUpdate: () => false }));
