...
describe("invalidateChatMessageQueries", () => {
  it("invalidates chat message cache", () => {
    const qc = createQueryClient();
...
describe("applyChatDoneToCache paged messages", () => {
  it("applies done payload to flat messages cache without duplicating replayed events", () => {
    const qc = createQueryClient();
    const existing = userMessage();
    qc.setQueryData<ChatMessage[]>(chatKeys.messages(sessionId), [existing]);

    applyChatDoneToCache(qc, donePayload());
    applyChatDoneToCache(qc, donePayload());

    const msgs = qc.getQueryData<ChatMessage[]>(chatKeys.messages(sessionId));

    expect(msgs?.map((m: ChatMessage) => m.id)).toEqual(["msg-user", "msg-assistant"]);
  });
});
